...
const SettingsNav: React.FC = () => {
  ...
  return (
    <nav className="bg-muted rounded-md text-card-foreground shadow-md w-full flex justify-center mb-10">
          ...
          {navLinks.map((link) => {
            ...
            return (
              <Link
                key={link.path}
                href={link.path}
                className={`flex items-center space-x-2 ${isActive ? 'bg-secondary' : 'hover:text-muted-foreground'} py-2 px-4 rounded-md`}
              >
                {link.icon}
